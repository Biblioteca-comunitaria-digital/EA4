...
class UserManager:
    ...
    def registrar_usuario(self, nombre_usuario: str, email: str, contraseña: str) -> tuple[bool, str]:
        ...
        if self.db_manager.ejecutar_consulta("SELECT id_usuario FROM usuarios WHERE nombre_usuario = %s", (nombre_usuario,)):
            return False, "El nombre de usuario ya está en uso."
        if self.db_manager.ejecutar_consulta("SELECT id_usuario FROM usuarios WHERE email = %s", (email,)):
            return False, "El email ya está en uso."

        # Rol estándar por defecto
        resultado_rol = self.db_manager.ejecutar_consulta("SELECT id_rol FROM roles WHERE nombre_rol = 'estandar'")
        if not resultado_rol:
            return False, "Error crítico: El rol 'estandar' no se encuentra en la base de datos."
        id_rol_estandar = resultado_rol[0]['id_rol']

        hash_contraseña = hashear_contraseña(contraseña)
        query = "INSERT INTO usuarios (nombre_usuario, email, contrasena_hash, id_rol) VALUES (%s, %s, %s, %s)"
    ...
    def cambiar_rol_usuario(self, id_usuario: int, nuevo_rol: str) -> bool:
        ...
        id_nuevo_rol = self.db_manager.ejecutar_consulta("SELECT id_rol FROM roles WHERE nombre_rol = %s", (nuevo_rol,))
        if not id_nuevo_rol:
            print(f"Error: El rol '{nuevo_rol}' no existe.")
            return False

        query = "UPDATE usuarios SET id_rol = %s WHERE id_usuario = %s"
